...
(* NEW: FLUSH SAME AS FENCE *)
| PredIsFlush : string -> FOLPredicateType
(* NEW: Get cacheability *)
| PredIsCacheable : string -> FOLPredicateType
...
Definition stringOfPredicate
  ...
  (* NEW: SAME AS FENCE *)
  | PredIsFlush a                        => StringOf ["IsFlush "; a]
  (* NEW: Add is cacheable*)
  | PredIsCacheable a                    => StringOf ["IsCacheable "; a]
...
Definition FOLLookupPredicate_S
  ...
  (* NEW: FLUSH SAME AS FENCE *)
  else if beq_string name "IsFlush"                        then PredIsFlush a
  (* NEW: Added pred to get cacheability *)
  else if beq_string name "IsCacheable"                    then PredIsCacheable a
