  ...
  try
    ...
    let accesses =
      try
          ...
          if ghost_auto
          then
          (* NEW: virtual here has cacheability now not sure if we need to care about true/false *)
          [Read (["dirtybit"], {vtag=10+addr; vindex=1; cacheability=1}, {ptag=PTETag addr; pindex=1},
              PipeGraph.PageTableEntry (addr, PipeGraph.PTag addr, {accessed=true; dirty=false}));
           Write (["dirtybit"], {vtag=10+addr; vindex=1; cacheability=1}, {ptag=PTETag addr; pindex=1},
              PipeGraph.PageTableEntry (addr, PipeGraph.PTag addr, {accessed=true; dirty=true}));
           Read (["RMW"], {vtag=addr; vindex=0; cacheability=1}, {ptag=PTag addr; pindex=0}, NormalData data);
           Read (["ptwalk"], {vtag=10+addr; vindex=1; cacheability=1}, {ptag=PTETag addr; pindex=1},
              PipeGraph.PageTableEntry (addr, PipeGraph.PTag addr, {accessed=true; dirty=true}))]
          else
          [Read (["RMW"], {vtag=addr; vindex=0; cacheability=1}, {ptag=PTag addr; pindex=0}, NormalData data)]
      with Not_found ->
      try
          ...
          then
          (* NEW: virtual here has cacheability now not sure if we need to care about true/false *)
          [Read ([], {vtag=addr; vindex=0; cacheability=1}, {ptag=PTag addr; pindex=0}, NormalData data);
           Read (["ptwalk"], {vtag=10+addr; vindex=1; cacheability=1}, {ptag=PTETag addr; pindex=1},
              PipeGraph.PageTableEntry (addr, PipeGraph.PTag addr, {accessed=true; dirty=false}));
           Read (["ptwalk"], {vtag=10+addr; vindex=1; cacheability=1}, {ptag=PTETag addr; pindex=1},
              PipeGraph.PageTableEntry (addr, PipeGraph.PTag addr, {accessed=true; dirty=true}))]
      ...
      try
        ...
        [Read (["dirtybit"], {vtag=10+addr; vindex=1; cacheability=1}, {ptag=PTETag addr; pindex=1},
         ...
         Write (["dirtybit"], {vtag=10+addr; vindex=1; cacheability=1}, {ptag=PTETag addr; pindex=1},
            PipeGraph.PageTableEntry (addr, PipeGraph.PTag addr, {accessed=true; dirty=true}));
         Write ([], {vtag=addr; vindex=0; cacheability=1}, {ptag=PTag addr; pindex=0}, NormalData data);
         Read (["ptwalk"], {vtag=10+addr; vindex=1; cacheability=1}, {ptag=PTETag addr; pindex=1},
            PipeGraph.PageTableEntry (addr, PipeGraph.PTag addr, {accessed=true; dirty=true}))]
        else
        [Write ([], {vtag=addr; vindex=0; cacheability=1}, {ptag=PTag addr; pindex=0}, NormalData data)]
      ...
      try
        ...
        let data = getData    (Str.matched_group 2 rw) in
        (* NEW: virtual here has cacheability now not sure if we need to care about true/false *)
        [Write (["RMW"], {vtag=addr; vindex=0; cacheability=1}, {ptag=PTag addr; pindex=0}, NormalData data)]
      with Not_found ->
        [Fence [rw]]
      in
