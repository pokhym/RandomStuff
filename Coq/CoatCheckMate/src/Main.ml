...
let cleanGhostInstructions h =
  ...
  | (Some va, Some pa) -> [
    ...
    {h with intraInstructionID0=1;
      (* NEW: Added cacheability not sure if bool value matters *)
       access=Read(["ptwalk"], {vtag=10+va.vtag; vindex=0; cacheability=1},
...
let dirtyGhostInstructions h =
  ...
  | (Some va, Some pa) -> [
    {h with intraInstructionID0=0;
    (* NEW: Added cacheability not sure if bool value matters *)
       access=Read(["dirtybit"], {vtag=10+va.vtag; vindex=0; cacheability=1},
         {ptag=PTETag va.vtag; pindex=0},
         PageTableEntry (va.vtag, pa.ptag,
           {accessed=true; dirty=false}))};
    {h with intraInstructionID0=1;
    (* NEW: Added cacheability not sure if bool value matters *)
       access=Write(["dirtybit"], {vtag=10+va.vtag; vindex=0; cacheability=1},
         {ptag=PTETag va.vtag; pindex=0},
         PageTableEntry (va.vtag, pa.ptag,
           {accessed=true; dirty=true}))};
    {h with intraInstructionID0=2};
